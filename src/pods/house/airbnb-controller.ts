...
import ListingModel from './listing-schema.js';
import { mapAirbnbFromApiToModel } from './airbnb.mappers.js';
import { getPaginationList } from './pagination.js';
...
export const getHouseDetail = async (req: Request, res: Response) => {
  ...
  try {
    const property: IListingAndReview | null = await ListingModel.findById(
      propertyId
    );
